...
import { 
  Send, Upload, Settings, RefreshCw, Cpu, Database, Award, 
  MessageSquare, FileText, CheckCircle, HelpCircle, HardDrive
} from 'lucide-react';
...
function App() {
  ...
  const [messages, setMessages] = useState([
    { role: 'assistant', content: 'Hi! I am DocuMind, your intelligent RAG assistant. Upload a text, markdown, or PDF file, adjust your RAG parameters, and ask me anything!', citations: [], latency_ms: null, cached: false }
  ]);
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        alert(`Successfully ingested file! Generated ${data.chunk_count} chunks.`);
        fetchDocuments();
      } else {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessages(prev => [...prev, {
          role: 'assistant',
          content: '⚠️ Rate limit exceeded. (FastAPI + Redis limit: Max 10 queries per minute allowed to prevent spam). Please wait 60 seconds.',
          citations: [],
          latency_ms: 0,
          cached: false
        }]);
      ...
      setMessages(prev => [...prev, {
        role: 'assistant',
        content: '⚠️ Server connection lost. Please ensure your FastAPI backend is running.',
        citations: [],
        latency_ms: 0,
        cached: false
      }]);
      ...
      <aside className="sidebar">
        ...
        <div className="panel-section">
          ...
          <input 
            ...
            onChange={handleFileUpload} accept=".pdf,.txt,.md"
          />
          <div className="upload-card" onClick={() => fileInputRef.current?.click()}>
            <Upload size={24} style={{ color: 'var(--accent-indigo)' }} />
            <p>{isUploading ? 'Ingesting document...' : 'Upload PDF, TXT, or MD'}</p>
          </div>
          
          <div style={{ marginTop: '0.5rem', maxHeight: '100px', overflowY: 'auto' }}>
            {uploadedDocs.map((doc, idx) => (
              <div key={idx} style={{ display: 'flex', gap: '8px', fontSize: '0.8rem', color: 'var(--text-secondary)', padding: '4px 0' }}>
        ...
        <div className="panel-section" style={{ marginTop: 'auto' }}>
          ...
          <div className="stats-grid">
            <div className="stat-card">
              ...
              <span className="stat-value" style={{ color: stats.redis_connected ? 'var(--accent-emerald)' : '#ef4444' }}>
            ...
            <div key={index} className={`message-wrapper ${msg.role}`}>
                  ...
                  <div className="citations-container">
                    {msg.citations.map((cite, cIdx) => (
                      <div 
                        ...
                        onClick={() => alert(`[CITATIONS EXCERPT]\nFile: ${cite.filename}\n\n"${cite.text_chunk}"`)}
                      >
                        <FileText size={12} />
                        <span>Source {cIdx + 1}: {cite.filename}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              {typeof msg.latency_ms === 'number' && (
                <div className="latency-badge">
                  ...
                  {msg.cached && <span className="cached">⚡ SERVED FROM REDIS RESPONSE CACHE</span>}
                </div>
              )}
            </div>
          ))}
